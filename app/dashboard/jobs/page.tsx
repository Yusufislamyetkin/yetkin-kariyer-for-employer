...
import { Plus, Briefcase, MapPin, DollarSign, Users, Edit, Trash2, Eye, X, Sparkles } from "lucide-react";
...
export default function JobsPage() {
  ...
  const handleExtractRequirements = async () => {
    if (!newJob.description.trim()) {
      alert("Lütfen önce açıklama girin");
      return;
    }

    setExtracting(true);
    try {
      const response = await fetch("/api/jobs/extract-requirements", {
        method: "POST",
      ...
      if (response.ok) {
        const data = await response.json();
        setNewJob({ ...newJob, requirements: data.requirements || [] });
      }
    } catch (error) {
  ...
  const handleCreateJob = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/jobs", {
        ...
        body: JSON.stringify({
          ...newJob,
          requirements: newJob.requirements.reduce((acc, req) => {
            acc[req] = true;
            return acc;
  ...
  const removeRequirement = (req: string) => {
    setNewJob({
      ...newJob,
      requirements: newJob.requirements.filter((r) => r !== req),
    });
  };

  const addRequirement = (req: string) => {
    if (req.trim() && !newJob.requirements.includes(req.trim())) {
      setNewJob({
        ...newJob,
        requirements: [...newJob.requirements, req.trim()],
      });
    }
  };
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {newJob.requirements.map((req) => (
                    <span
                      key={req}
                      className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400"
                    >
                      {req}
                      <button
                        type="button"
                ...
                <Input
                  placeholder="Teknoloji ekle (örn: React, Node.js)"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
